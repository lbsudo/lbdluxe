...
var require_version = __commonJS({
  "../node_modules/uuid/dist/commonjs-browser/version.js"(exports) {
    ...
    __name(version6, "version");
    var _default = exports.default = version6;
  }
});
    ...
    var Webhook2 = class _Webhook {
      ...
      verify(payload, headers_) {
        ...
        for (const versionedSignature of passedSignatures) {
          const [version6, signature] = versionedSignature.split(",");
          if (version6 !== "v1") {
            continue;
          }
...
// src/middleware/resendClient.ts
var resendClientMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  await next();
}, "resendClientMiddleware");

// dist/server/src/routes/resend/add-resend-contact.js
...
// src/routes/resend/get-all-resend-contacts.ts
...
// dist/server/src/routes/resend/index.js
...
// src/middleware/supabaseClient.ts
var supabaseMiddleware = /* @__PURE__ */ __name(async (c, next) => {
...
// src/routes/supabase/profile/get-profile.ts
var getProfile = new Hono2();
...
// src/routes/supabase/profile/get-profile-images.ts
var getProfileImages = new Hono2();
getProfileImages.get("/", async (c) => {
  try {
    const supabase = c.var.supabase;
    const { data: files, error } = await supabase.storage.from("profile-images").list("", {
      limit: 1e3,
      // adjust as needed
      offset: 0,
      sortBy: { column: "name", order: "asc" }
    ...
    const imageUrls = files.map(
      (file) => supabase.storage.from("profile-images").getPublicUrl(file.name).data.publicUrl
    );
...
// src/routes/supabase/profile/update-profile.ts
var updateProfile = new Hono2();
updateProfile.post("/", async (c) => {
  try {
    const supabase = c.var.supabase;
    const { description, words } = await c.req.json();
    const { data: existing, error: fetchError } = await supabase.from("profile").select("id").limit(1).maybeSingle();
    if (fetchError) throw fetchError;
    if (!existing) return c.json({ success: false, error: "Profile not found" }, 404);
    const { error: updateError } = await supabase.from("profile").update({ description, words }).eq("id", existing.id);
    if (updateError) {
...
// src/routes/supabase/profile/delete-profile-image.ts
var deleteProfileImage = new Hono2();
deleteProfileImage.delete("/:filename", async (c) => {
  try {
    const supabase = c.var.supabase;
    const { filename } = c.req.param();
    const filenameSchema = external_exports.string().min(1, "Filename required");
...
// src/routes/supabase/profile/update-profile-image.ts
var updateProfileImage = new Hono2();
updateProfileImage.post("/", async (c) => {
  try {
    ...
    if (fetchError) throw fetchError;
    if (!existing) return c.json({ success: false, error: "Profile not found" }, 404);
    const { error: updateError } = await supabase.from("profile").update({ profile_image_url }).eq("id", existing.id);
    if (updateError) {
      console.error("Supabase update error:", updateError);
      return c.json({ success: false, error: updateError.message }, 500);
    }
...
// src/routes/supabase/profile/upload-profile-image.ts
var uploadProfileImage = new Hono2();
...
// src/routes/supabase/index.ts
var supabaseRoutes = new Hono2();
...
var app = new Hono2();
console.log("\u{1F680} Dev server running at http://localhost:8787");
app.use("/*", cors({
...
// .wrangler/tmp/bundle-gEPSEK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gEPSEK/middleware-loader.entry.ts
