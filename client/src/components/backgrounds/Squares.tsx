...
const Squares: React.FC<SquaresProps> = ({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      canvas.width = window.innerWidth; // Set canvas width to full viewport
      canvas.height = window.innerHeight; // Set canvas height to full viewport
      numSquaresX.current = Math.ceil(canvas.width / squareSize) + 1;
      numSquaresY.current = Math.ceil(canvas.height / squareSize) + 1;
    };

    window.addEventListener('resize', resizeCanvas);
    resizeCanvas(); // Initial setup for full page coverage
  ...
  return (
    <div
      style={{
        ...
        overflow: 'hidden', // Avoid scroll issues
      }}
    >
      <canvas
        ref={canvasRef}
