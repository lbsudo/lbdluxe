...
export const Route = createFileRoute('/admin/blog/edit-post/$postId')({
...
function RouteComponent() {
    ...
    useEffect(() => {
        if (post && authors.length > 0 && categories.length > 0) {
            ...
            // Map tag names to category IDs
            const categoryIds = categories
                .filter(cat => post.tags.includes(cat.name))
                .map(cat => cat.id);
    ...
    const { mutate: updatePost, isPending } = useMutation({
        mutationFn: async () => {
            ...
            const res = await fetch(`${SERVER_URL}/api/updateBlogPost/${postId}`, {
                method: "PUT",
    ...
    return (
        <AdminLayout path="/admin/blog/edit-post">
            {isLoading ? (
                ...
                <div className="flex flex-col gap-4 w-full py-4">
                    ...
                    <Button onClick={() => updatePost()} disabled={isPending}>
                        {isPending ? "Updating..." : "Update Blog Post"}
                    </Button>
                </div>
            ) : (
